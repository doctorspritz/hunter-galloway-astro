const dataCalculateValue = +document.querySelector('[data-calculate-value]')
  ?.dataset.calculateValue;
...
window.addEventListener('scroll', (e) => {
  ...
  const sdCalculatorSection = document.querySelector('#sd-calculator-section');
  const dpCalculatorSection = document.querySelector('#dp-calculator-section');

  if (!sdCalculatorSection && !dpCalculatorSection) return;
  if (
    sdCalculatorSection?.getBoundingClientRect().bottom < 0 ||
    (dpCalculatorSection?.getBoundingClientRect().bottom < 0 &&
  ...
  if (isMobile) {
    ...
    const newDepositCalculator = document.querySelector(
      '#dp-calculator-section'
    );

    if (newDepositCalculator) {
      const footerButton = document.querySelector('.footer_button');
      if (!dpCalculatorSection || !footerButton) return;
      const rect = dpCalculatorSection.getBoundingClientRect();
      footerButton.hidden = rect.top < window.innerHeight && rect.bottom > 0;
    }
  } else {
    if (
...
if (dataCalculateValue != 1) {
  document
    .querySelectorAll('#et-main-area .btn_calculate:not(#dp-entry-calculate)')
    .forEach((item) => {
      item.addEventListener('click', (e) => {
        e.preventDefault();
        ...
        if (
          document.querySelector(
            '.dp-calculator__step[data-step="2"][style*="display: block"]'
          )
        ) {
          jQuery(function ($) {
            $('html, body').animate(
              {
                scrollTop: $('.dp-form').offset().top - 75,
              },
              250
            );
          });
          ...
          if (value != '') {
            const depositInputs = document.querySelectorAll('#hcdc_deposit');
            if (!depositInputs || depositInputs.length === 0) return;
            depositInputs.forEach((input) => (input.value = value));
          }
        } else if (
          document.querySelector(
        ...
        } else if (document.querySelector('.sd-calculator')) {
          if (value !== '') {
            const stampDutyPropertyPriceInput = document.querySelector(
              '#connective-htmlcw-pf-property-price'
            );
            if (stampDutyPropertyPriceInput) {
              stampDutyPropertyPriceInput.value = value;
            }
          }
          jQuery(function ($) {
            $('html, body').animate(
              {
                scrollTop: $('.sd-calculator').offset().top - 171,
              },
              250
            );
          });
...
const globalMutation = new MutationObserver((mutations) => {
  if (!document.querySelector('.elementor-editor-active')) {
    ...
    if (localStorage.getItem('scrollTo') != null) {
      ...
      } else if (document.querySelector('#entry-deposit')) {
        if (value !== '') {
          document.querySelector('#entry-deposit').value = value;
        }
