...
function view_microdata($microdata_model) {
	if ($microdata_model->test_mode) echo "<!-- keradan microdata here: -->" . PHP_EOL;
	if ($microdata_model->test_mode) echo "<!-- current_page: " . $microdata_model->current_page . " -->" . PHP_EOL;
	foreach ($microdata_model->pages->{$microdata_model->current_page} as $object_name) {
		$object = $microdata_model->objects->{$object_name};
		$object->{"@context"} = "https://schema.org";
		
		echo '<script type="application/ld+json">' . PHP_EOL;
		if ($microdata_model->test_mode) echo json_encode($object, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
		else echo json_encode($object);
		echo PHP_EOL . '</script>' . PHP_EOL;
	}
	if ($microdata_model->test_mode) echo "<!-- keradan microdata END. -->";
}

function get_microdata_current_page() {
	if (is_front_page()) return 'main';
	if (is_page('mortgage-broker-brisbane')) return 'faq_mbb';
	if (is_page('first-home-buyer-loans')) return 'faq_fhb';
	if (is_page('home-loans-for-doctors')) return 'faq_doctors';
	if (is_page('contact')) return 'contact_us';
	if (is_single()) return 'posts';
	if (is_page()) return 'pages';
	if (is_author()) return 'authors_page';
	return 'any';
}

$microdata_model = (object)[
	'test_mode' => true,
...
view_microdata($microdata_model);
