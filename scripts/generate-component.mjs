...
`---
export interface Props { variant?: 'sm'|'md'|'lg' }
const { variant = 'md' } = Astro.props;
---
<style define:vars={{ /* use tokens here */ }}>
</style>
...
try { await fs.access('docs/COMPONENTS.md'); } catch {
  await fs.writeFile('docs/COMPONENTS.md',
`# Component Registry
...
await fs.appendFile('docs/COMPONENTS.md',
`| ${Name} | ${level} | ${compPath} | sm,md,lg |  |\n`);
